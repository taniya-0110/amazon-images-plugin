...
class ChatGPTAutomation {
  constructor() {
    ...
    this.tempImageFolder = path.join(__dirname, 'temp_images');
    this.generatedImagesFolder = path.join(__dirname, 'generated_images');
    this.profileFolder = path.join(__dirname, 'playwright-profile1');
    this.resultsFile = path.join(__dirname, 'analysis_results.json');
    this.statusFile = path.join(__dirname, 'status.json');
    this.continueSignalFile = path.join(__dirname, 'continue_signal.json');
...
Wait for the next instruction before generating Image ${imageNumber + 1}.`;
  ...
  async saveLastGeneratedImage(page, imageNumber, previousSources = []) {
    try {
      ...
      if (!candidate) {
        console.log('  ⚠️ No new generated image found');
        return null;
      }
      ...
      try {
        ...
        console.log('  ⚠️ Browser image fetch failed, trying axios:', browserFetchError.message);
      ...
      if (!buffer || buffer.length < 1024) {
        console.log('  ⚠️ Image buffer too small or empty');
        return null;
      }
    ...
    try {
      ...
      results.detailedAnalysis.forEach(img => {
        ...
        if (img.reasonsForDeductedMarks.length > 0) {
          ...
          console.log(`    ⚠️ No reasons found for Image ${img.imageNumber}`);
        }
      });
      ...
      if (results.detailedAnalysis.every(img => img.reasonsForDeductedMarks.length === 0)) {
        console.log('⚠️ No reasons found with standard parsing. Trying alternate parsing...');
...
module.exports = new ChatGPTAutomation();
